#include "domain/travelers/FrequentFlyerAccount/FrequentFlyerAccount.h"

void FrequentFlyerAccount::configureFrequentFlyerAccount(const std::shared_ptr<PartnerAirline>& partner, int delta) {
    linkedPartner = partner;
    priorityLevel += delta;
    statusLabel = internalNote();
    if (linkedPartner) { statusLabel += "-linked"; }
}

std::string FrequentFlyerAccount::describeFrequentFlyerAccount() const {
    const bool attached = static_cast<bool>(linkedPartner);
    return statusLabel + "-" + std::to_string(priorityLevel) + (attached ? "-ready" : "-solo");
}

std::string FrequentFlyerAccount::internalNote() const {
    return statusLabel + "-" + std::string(linkedPartner ? "partnered" : "independent");
}