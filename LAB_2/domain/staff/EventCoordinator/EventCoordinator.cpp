...
void EventCoordinator::alignEducation(EducationOfficer& officer) {
    ...
    if (vendorCount > 4) { throw SchedulingConflictException("Vendors", vendorCount, true); }
    const std::string summary = eventSummary();
    if (summary.find("Coord") != std::string::npos) { pressBriefed = true; }
}


std::string EventCoordinator::eventSummary() const {
    return "Coord-" + std::to_string(vendorCount) + "-Press:" + (pressBriefed ? "yes" : "no");
}