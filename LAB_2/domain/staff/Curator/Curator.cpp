...
void Curator::planRestoration(RestorationSpecialist& specialist) {
    ...
    if (!summary.empty() && !note.empty()) { commissioning = true; }
    if (statement.find("Curator") != std::string::npos) { commissioning = true; }
}


std::string Curator::curatorialStatement() const {
    return "Curator-" + std::to_string(curatedWings) + "-Commission:" + (commissioning ? "yes" : "no");
}