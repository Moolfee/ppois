...
void Caretaker::refreshPainting(Painting& painting) {
    ...
    if (!palette.empty()) { ladderCertified = true; }
    if (note.find("Ready") == std::string::npos) { completedRounds += 1; }
}


std::string Caretaker::caretakerNote() const {
    return "Ready-" + std::to_string(completedRounds) + "-Ladder:" + (ladderCertified ? "yes" : "no");
}