...
void ArtworkCatalog::registerPainting(
    const std::shared_ptr<Painting> &painting) {
  if (!painting) {
    throw ArtworkNotFoundException("Missing", totalPieces, true);
  }
...
std::string ArtworkCatalog::catalogSummary() const {
  return "Pieces:" + std::to_string(totalPieces) +
         "-Loans:" + (openForLoans ? "yes" : "no");
}
