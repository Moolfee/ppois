...
inline void ResetFailures() { FailureCount() = 0; }
...
struct TestRegistrar {
  TestRegistrar(std::string suite, std::string name,
                std::function<void()> body) {
    Registry().push_back(
        TestCase{std::move(suite), std::move(name), std::move(body)});
  }
};
...
inline int RunAllTests() {
  ...
  std::cout << "\nSummary: " << passed << " passed, " << failedSuites
            << " failed.\n";
  return failedSuites == 0 ? 0 : 1;
}

inline void ExpectTrue(bool condition, const char *expression, const char *file,
...
inline void ExpectBinary(bool condition, const LHS &lhs, const RHS &rhs,
                         const char *lhsText, const char *rhsText,
                         const char *op, const char *file, int line) {
  if (!condition) {
    std::ostringstream oss;
    oss << lhsText << " " << op << " " << rhsText << " (" << lhs << " vs "
        << rhs << ")";
...
#define EXPECT_TRUE(condition)                                                 \
  ::testing::ExpectTrue(static_cast<bool>(condition), #condition, __FILE__,    \
                        __LINE__)

#define EXPECT_EQ(val1, val2)                                                  \
  ::testing::ExpectBinary(((val1) == (val2)), val1, val2, #val1, #val2,        \
                          "==", __FILE__, __LINE__)
